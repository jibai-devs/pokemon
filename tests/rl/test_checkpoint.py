...
def test_checkpoint_roundtrip(tmp_path):
    ...
    template = net.init_params(
        model, jax.random.PRNGKey(1), features.STATE_DIM, features.OPTION_DIM
    )
    ...
    assert len(a) == len(b)
    assert all(np.allclose(np.asarray(x), np.asarray(y)) for x, y in zip(a, b, strict=True))
